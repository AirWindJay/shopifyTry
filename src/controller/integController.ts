...
async function getIntegId(id, storeName) {
    const integSettings = await prisma.integration_settings.findFirst({
        where: {
            value: storeName,
        }
    })
    const integ = await prisma.integrations.findFirst({
        where: {
            user_id: id,
            marketplace_id: 1,
            id: integSettings.integration_id
        }
    })
    if(integ !== null) {
        return integ.id
...
// export default {getUserId, getIntegId, checkIntegWoo, saveInteg, processInteg, deleteIntegration, updateInteg, test}
export default {getUserId, getIntegId}